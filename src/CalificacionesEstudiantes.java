public class CalificacionesEstudiantes {
    ...
    public static void encontrarMejorEstudiante(String[] estudiantes, double[][] notas) {
        ...
        System.out.println("\nEl mejor estudiante es: " + estudiantes[mejorEstudianteIndex] + " con un promedio de " + mejorPromedio);
    }

    public static void encontrarMejorMateria(String[] materias, double[][] notas) {
        double[] promedioMaterias = new double[materias.length];
        ...
        for (int j = 0; j < materias.length; j++) {
            ...
            promedioMaterias[j] = suma / notas.length;
        }

        int mejorMateriaIndex = 0;
        double mejorPromedioMateria = promedioMaterias[0];

        for (int j = 1; j < promedioMaterias.length; j++) {
            if (promedioMaterias[j] > mejorPromedioMateria) {
                mejorPromedioMateria = promedioMaterias[j];
                mejorMateriaIndex = j;
            }
        }

        System.out.println("\nLa materia con el promedio más alto es: " + materias[mejorMateriaIndex] + " con un promedio de " + mejorPromedioMateria);
    }
}
